...
it("discovers supporting documents and refuses paths outside their workspace", () => {
  ...
  writeFileSync(join(cwd, "memory.md"), "Incidental workspace note.");
  ...
  const artifacts = collectChannelArtifacts([{ threadId: "research", cwd, output: "recommendation.md" }], 0);
  expect(artifacts[0]).toEqual({ label: "recommendation.md", path: join(cwd, "recommendation.md"), threadId: "research" });
