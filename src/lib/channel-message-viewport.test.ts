...
describe("channelViewportRows", () => {
  ...
  it("filters duplicated sourced activity and recognizes legacy Coordinator messages", () => {
    const legacy = message({ id: "legacy", executionReport: "done", from: undefined });
    const rows = channelViewportRows([
  ...
  it("keeps a direct worker delivery in its sender cluster despite its execution report", () => {
    const delivery = message({ id: "direct", executionReport: "completed" });
    const rows = channelViewportRows([
      message({ id: "worker" }),
      delivery,
      message({ id: "system", author: "coordinator", from: undefined }),
    ]);

    expect(isCoordinatorMessage(delivery)).toBe(false);
    expect(rows.map((row) => row.showCluster)).toEqual([true, false, true]);
  });
});
